...
function is_user_edit_status(){
    if($("#user_table_div table tbody tr.user_edit_status").length > 0){
        art.dialog({id: 'bktips', width: 300,icon: 'error',lock: true,content: '不可同时修改多个用户信息，请先保存编辑中的用户信息'}).time(5);
        return true;
    }
    return false;
}
...
function fetch_role(){
    // 有记录id则进行修改
    var url = site_url + 'api/role/users/';
    var select_option = '';
    $.ajax({
        url: url,
        type: "GET",
        data: {
            role_type__in: 'GENERAL,ADMIN'
        },
        async: false,
        success: function(data){
            if(data.result){
                data.data.reverse().forEach(role => {
                    select_option += `<option value="${role.role_key}">${role.name}</option>\n`;
                });
            }else{
...
$('.user_add_btn').on('click',function(){
    ...
    var tpl = [
            ...
            '   <td>',
            '       <input class="form-control u_username" placeholder="'+  '请输入用户名' +'"/>',
            '   </td>',
            '    <td>',
            '        <input class="form-control u_chname" placeholder="'+  '请输入姓名' +'"/>',
            '   </td>',
            '   <td>',
            '       <input class="form-control u_phone" placeholder="'+  '请输入联系电话' +'"/>',
            '   </td>',
            '   <td>',
            '       <input class="form-control u_email" placeholder="'+  '请输入邮箱' +'"/>',
            '   </td>',
            '   <td>',
            ...
            '       <a href="###" title="'+  '删除' +'" class="dev_user_opera user_del_btn"><span aria-hidden="true" class="glyphicon glyphicon-remove-circle"></span></a>',
            '      <a href="###" title="'+  '重置密码' +'" class="dev_user_opera user_rest_btn"><span aria-hidden="true" class="glyphicon glyphicon-lock"></span></a>',
            '   </td>',
            '</tr>'
              ].join('');
        ...
        // $('#add_user_role').append(fetch_role());
    return false;
});
// 导出用户数据
...
$('#user_table_div').on('click','.user_save_btn',function(){
    ...
    if (!u_username.match(/^[A-Za-z0-9][A-Za-z0-9._]{2,18}[A-Za-z0-9]$/)){
        art.dialog({id: 'bktips', width: 300,icon: 'error',lock: true,content:  '用户名只能包含数字、字母、下划线和点，且长度在4-20个字符, 且必须以字母或数字开头'});
        curRecord.find('.u_username').focus();
        return false;
    }
    if (!u_chname.match(/^[\u4e00-\u9fa5a-zA-Z0-9_]{1,16}$/)){
        art.dialog({id: 'bktips', width: 300,icon: 'error',lock: true,content:  '中文名只能包含数字、字母、中文汉字、下划线，长度在1-16个字符'});
        curRecord.find('.u_chname').focus();
        return false;
    }
    if (!u_phone.match(/^\d{10,11}$/)){
        art.dialog({id: 'bktips', width: 300,icon: 'error',lock: true,content:  '仅支持中国大陆手机号码（11位数字）'});
        curRecord.find('.u_phone').focus();
        return false;
    }
    if (!u_email.match(/^[A-Za-z0-9]+([-_.][A-Za-z0-9]+)*@([A-Za-z0-9]+[-.])+[A-Za-z0-9]{2,5}$/)){
        art.dialog({id: 'bktips', width: 300,icon: 'error',lock: true,content:  '请输入正确的邮箱格式'});
        curRecord.find('.u_email').focus();
        return false;
    }
    // if (!u_role){
    //     art.dialog({id: 'bktips', width: 300,icon: 'error',lock: true,content:  '请选择角色'});
...
$('#user_table_div').on('click','.user_edit_btn',function(){
    ...
    // 编辑时修改 placeholder
    curRecord.find('.u_username').attr('placeholder',  '请输入用户名');
    curRecord.find('.u_chname').attr('placeholder',  '请输入中文名');
    curRecord.find('.u_phone').attr('placeholder',  '请输入手机号');
    curRecord.find('.u_email').attr('placeholder',  '请输入邮箱');
...
$('#user_table_div').on('click','.user_del_btn',function(){
    ...
    if (user_id){
        var url = site_url + 'accounts/user/' + user_id + '/';
        var content = "<div class='t_s14'>" +  "您确定删除该用户吗?" + "<br>" +  "用户名 : "+ u_username + "</div>";
        var width = 340;
        art.dialog({
            title:  "删除确认",
            width: width,
                ...
                if(!flag){
                    ...
                    if(!post_flag){
                      return false;
                        ...
                        art.dialog({width: 300,icon: 'succeed',lock: true,content:  '密码重置成功'}).time(2);
                    }
                }
            }
        });
