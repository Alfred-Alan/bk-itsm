...
from blueapps.account.constants import ApiErrorCodeEnumV2
...
class AllUsersView(LoginExemptMixin, View):
    def get(self, request):
        ...
        # 获取用户信息
        data = User.objects.get_all_users_v2(role)
        return ApiV2OKJsonResponse(_("用户信息获取成功"), data=data)
...
class BatchUsersView(CsrfAndLoginExemptMixin, View):
    def post(self, request):
        ...
        # 获取用户信息
        data = User.objects.get_batch_users_with_dict_v2(username_list)
        return ApiV2OKJsonResponse(_("用户信息获取成功"), data=data)
