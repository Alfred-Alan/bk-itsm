...
from common.mixins.exempt import LoginExemptMixin, CsrfExemptMixin
from blueapps.account.forms import PasswordForm, BaseUserInfoForm, WeixinInfoForm
...
class UserView(LoginExemptMixin, View):
    def get(self, request):
        ...
        # 获取用户数据
        result, data, message = User.objects.get_user_info_v2(username)
        if not result:
...
class BatchUsersView(CsrfAndLoginExemptMixin, View):
    def post(self, request):
        ...
        # 获取用户信息
        data = User.objects.get_batch_user_with_dict(username_list)
        return ApiV1OKJsonResponse(_("用户信息获取成功"), data=data)
...
class CurrentUserPasswordView(CsrfExemptMixin, View):
    def put(self, request):
        ...
        result, error_code, message = User.objects.modify_password_by_username(request.user.username,
...
class CurrentUserBaseInfoView(CsrfExemptMixin, View):
    def post(self, request):
        ...
        result, error_code, message = User.objects.modify_user_info(request.user.username,
...
class CurrentUserWeixinInfoView(CsrfExemptMixin, View):
    def post(self, request):
        ...
        result, message = User.objects.bind_wx_user_info(request.user, form.cleaned_data["wx_userid"])
    ...
    def delete(self, request):
        result, message = User.objects.unbind_wx_user_info(request.user)
