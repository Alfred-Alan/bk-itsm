...
from blueapps.account.encryption import encrypt, decrypt, salt
from blueapps.account.models import Loignlog, BkToken
...
class Account(AccountSingleton):
    ...
    def get_bk_token(self, username):
        ...
        while not bk_token and retry_count < 5:
            ...
            try:
                ...
                logger.exception(
                    'Login ticket failed to be saved during ticket generation, error: {}'.format(
                        error))
    ...
    def login(self, request, template_name='login/login.html',
        ...
        登录页面和登录动作
        """
        if re.match(r".*/plain/$", request.path):
            template_name = 'login/plain_login.html'
    ...
    def login_failed_response(self, request, redirect_to, app_id):
        ...
        if app_id:
            query['app_id'] = app_id
        if len(query):
            redirect_url = "{}?{}".format(self.BK_LOGIN_URL, urlencode(query))
